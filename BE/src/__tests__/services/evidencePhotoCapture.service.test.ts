...
const { mockFindByHash, mockUpload } = vi.hoisted(() => ({ mockFindByHash: vi.fn(), mockUpload: vi.fn() }));
vi.mock('../../repositories/evidencePhotoRegistryRepository', () => ({ findEvidencePhotoRegistryBySha256FromRepository: mockFindByHash }));
vi.mock('../../services/projectService', () => ({ uploadProjectEvidenceFileToPinataWithRetry: mockUpload }));

import { processCapturedEvidencePhotos } from '../../services/evidencePhotoCapture.service';
...
describe('processCapturedEvidencePhotos', () => {
  ...
  it('pins a valid JPEG and returns normalized evidence metadata', async () => {
    await expect(processCapturedEvidencePhotos({ photos: [photo], module: 'PROJECT_CHALLENGE', ownerUserId: 'auditor-1', clientSubmittedAt: submittedAt.toISOString(), serverReceivedAt: submittedAt })).resolves.toMatchObject([{ cid: 'bafy-photo', accuracyMeters: 20, contentSha256: expect.any(String) }]);
  });
