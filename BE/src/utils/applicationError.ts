export type ApplicationErrorCode =
  | 'VALIDATION_ERROR'
  | 'UNAUTHENTICATED'
  | 'FORBIDDEN'
  | 'CONFLICT'
  | 'NOT_FOUND'
  | 'INVALID_STATUS_TRANSITION'
  | 'CHAIN_MISMATCH'
  | 'TRANSACTION_TIMEOUT'
  | 'TRANSACTION_REVERTED'
  | 'EVENT_NOT_FOUND'
  | 'PROJECT_NOT_FOUND'
  | 'INVALID_TICKET'
  | 'TICKET_EXPIRED'
  | 'TICKET_ALREADY_USED'
  | 'DUPLICATE_SUBMISSION'
  | 'CAPTCHA_FAILED'
  | 'DUPLICATE_LEGAL_REGISTRATION_NUMBER'
  | 'UNSUPPORTED_MEDIA_TYPE'
  | 'PROJECT_NOT_ACCEPTING_FEEDBACK'
  | 'RATE_LIMIT_EXCEEDED'
  | 'PROJECT_NOT_ACTIVE'
  | 'AMOUNT_INVALID'
  | 'UNAUTHORIZED_RELAYER'
  | 'CONTRACT_REVERTED'
  | 'SMART_ACCOUNT_MISMATCH'
  | 'INSUFFICIENT_TOKEN_BALANCE'
  | 'PAYMASTER_POLICY_MISMATCH'
  | 'MINIMUM_RAISED_NOT_MET'
  | 'MAX_WITHDRAWAL_EXCEEDED'
  | 'DUPLICATE_BENEFICIARY_PENDING'
  | 'REQUEST_EXPIRED'
  | 'ALREADY_SIGNED'
  | 'INTERNAL_ERROR'
  | 'BLOCKCHAIN_UNAVAILABLE'
  | 'GUEST_SESSION_NOT_FOUND'
  | 'GUEST_SESSION_NOT_ACTIVE'
  | 'GUEST_SESSION_EXPIRED'
  | 'GUEST_SESSION_ALREADY_INACTIVE'
  | 'GUEST_DONATION_QUOTA_EXCEEDED'
  | 'GUEST_AMOUNT_LIMIT_EXCEEDED'
  | 'INVALID_CALLDATA'
  | 'DUPLICATE_USEROP'
  | 'GUEST_TOKEN_REQUIRED'
  | 'GUEST_TOKEN_INVALID'
  | 'GUEST_SESSION_REQUIRED'
  | 'GUEST_RENEWAL_LIMIT_EXCEEDED'
  | 'GUEST_SESSION_RATE_LIMIT_EXCEEDED'
  | 'GUEST_DONATION_RATE_LIMIT_EXCEEDED'
  | 'INVALID_WALLET_ADDRESS'
  | 'INVALID_FINGERPRINT'
  | 'GUEST_SESSION_LIMIT_EXCEEDED'
  | 'GUEST_IP_BURST_DETECTED'
  | 'GUEST_WALLET_MISMATCH'
  | 'GUEST_CLAIM_NONCE_INVALID'
  | 'GUEST_CLAIM_NONCE_EXPIRED'
  | 'GUEST_CLAIM_ALREADY_USED'
  | 'GUEST_CLAIM_SESSION_NOT_ACTIVE'
  | 'GUEST_CLAIM_ALREADY_PREPARED'
  | 'CLAIM_SESSION_MISMATCH'
  | 'BUNDLER_SUBMISSION_FAILED'
  | 'BUNDLER_NETWORK_ERROR'
  | 'BUNDLER_HTTP_ERROR'
  | 'BUNDLER_HTTP_CLIENT_ERROR'
  | 'BUNDLER_INVALID_RESPONSE'
  | 'BUNDLER_RPC_ERROR'
  | 'GUEST_CLAIM_ALREADY_USED_VALIDATION'
  | 'INVALID_SESSION_TOKEN'
  | 'RELAY_NOT_AVAILABLE'
  | 'AMOUNT_TOO_LARGE'
  | 'ENCRYPTION_FAILED'
  | 'DONATION_QUOTA_EXCEEDED'
  | 'TOTAL_AMOUNT_EXCEEDED'
  | 'PENDING_DONATION_EXISTS'
  | 'DECRYPTION_ERROR'
  | 'TRANSACTION_FAILED'
  | 'BATCH_SIZE_EXCEEDED'
  | 'EMPTY_BATCH'
  | 'FILE_TOO_LARGE'
  | 'INVALID_CSV_FORMAT'
  | 'PAYLOAD_MUST_BE_ARRAY'
  | 'FEEDBACK_ALREADY_DELETED'
  | 'FEEDBACK_NOT_FLAGGED'
  | 'FEEDBACK_NOT_DELETED'
  | 'ORACLE_AUTH_UNAVAILABLE'
  | 'ORACLE_SIGNATURE_INVALID'
  | 'ORACLE_SIGNATURE_REPLAY'
  | 'VERIFICATION_NOT_VALID'
  | 'VERIFICATION_DATA_INCOMPLETE'
  | 'VERIFICATION_DATA_MISMATCH'
  | 'BENEFICIARY_NOT_FOUND'
  | 'VERIFICATION_NOT_FOUND'
  | 'MILESTONE_PLAN_REQUIRED'
  | 'GEOFENCE_REQUIRED'
  | 'MILESTONE_NOT_FOUND'
  | 'CHALLENGE_WINDOW_CLOSED'
  | 'ALREADY_VOTED'
  | 'ARBITRATION_INTEGRITY_ERROR'
  | 'DONATION_LOCK_RISK_NOT_ACKNOWLEDGED'
  | 'NOT_COMMITTEE_MEMBER'
  | 'COMMITTEE_ROSTER_INVALID'
  | 'GOVERNANCE_SEAT_MIGRATION_REQUIRED'
  | 'FIELD_REPORT_ALREADY_EXISTS'
  | 'DUPLICATE_EVIDENCE_PHOTO'
  | 'CLOCK_SKEW_TOO_LARGE'
  | 'CAPTURE_EXPIRED'
  | 'LOW_ACCURACY_REASON_REQUIRED'
  | 'INTENT_NOT_FOUND'
  | 'INVALID_WALLET'
  | 'ALREADY_SUBMITTED'
  | 'EMAIL_EXISTS'
  | 'AUDITOR_ONBOARDING_NOT_FOUND'
  | 'ALREADY_AUDITOR'
  | 'ONBOARDING_RESUME_UNAVAILABLE'
  | 'AMOUNT_BELOW_MINIMUM_FLOOR'
  | 'FULL_EXIT_NOT_ELIGIBLE';

/**
 * Hàm lớp lỗi nghiệp vụ dùng chung.
 * Mục đích: chuẩn hóa mã lỗi và HTTP status cho toàn bộ ứng dụng.
 */
export class ApplicationError extends Error {
  public readonly statusCode: number;

  public readonly errorCode: ApplicationErrorCode;

  constructor(message: string, statusCode: number, errorCode: ApplicationErrorCode) {
    super(message);
    this.name = 'ApplicationError';
    this.statusCode = statusCode;
    this.errorCode = errorCode;
  }
}
