...
// =============================================================================
// TimelineCard cho System Admin Page
// Clone from: FE/app/components/regulatoryBodies/tailwind/TimelineCard.tsx
// Mục đích: Hiển thị timeline hoạt động gần đây với icon-coded event types
// =============================================================================

import type { TimelineItem } from './types';

/** Hàm trả về màu nền và viền icon theo loại sự kiện. */
...
/** Hàm trả về icon SVG theo loại sự kiện. */
      ...
      <ul>
        {timelineItemList.map((timelineItem, index) => (
          <li key={`${timelineItem.actionText}-${timelineItem.timeText}`} className="relative flex items-start gap-3 border-b border-slate-100 px-5 py-3.5 last:border-b-0">
            {/* Vertical connector line giữa các item */}
            {index < timelineItemList.length - 1 ? (
              <span className="absolute left-[33px] top-[42px] h-[calc(100%-30px)] w-px bg-slate-200" />
            ) : null}
            {/* Icon circle */}
            <span className={`relative z-10 inline-flex h-7 w-7 items-center justify-center rounded-full border ${getTimelineToneClassName(timelineItem.type)}`}>
              <svg viewBox="0 0 16 16" className="h-3.5 w-3.5" aria-hidden="true">
                {getTimelineIcon(timelineItem.type)}
              </svg>
            </span>
            {/* Content */}
