...
import { requiresRiskAcknowledgement, type ExecutiveDeviationLevel } from '@/app/utils/executiveCommitteePolicy';

type DeviationLevel = ExecutiveDeviationLevel;
...
export function DisbursementVotingPanel(): ReactElement {
  ...
  const loadCases = useCallback(async (cursor: string | null = null, append = false): Promise<void> => {
    ...
    try {
      ...
      const response = await fetchApi<PendingDisbursementResponse>(
        buildSameOriginApiUrl(`/api/disbursement/executive/pending?${searchParameters.toString()}`),
  ...
  const vote = async (requestId: string, decision: 'APPROVE' | 'REJECT'): Promise<void> => {
    ...
    try {
      const signingPayloadResponse = await fetchApi<CommitteeVoteSignaturePayload | null>(
        buildSameOriginApiUrl('/api/disbursement/executive/' + encodeURIComponent(requestId) + '/signing-payload'),
      ...
      const eip712Signature = signingPayloadResponse.data
        ...
        : undefined;
      await fetchApi(buildSameOriginApiUrl('/api/disbursement/executive/' + encodeURIComponent(requestId) + '/vote'), {
        method: 'POST',
        headers: getVotingHeaders(),
  ...
  return <section aria-labelledby="disbursement-title" className="min-w-0 rounded-3xl border border-slate-200/80 bg-white/90 p-4 shadow-[0_16px_35px_-28px_rgba(15,23,42,0.45)] backdrop-blur sm:p-6">
    <header className="flex flex-col gap-4 border-b border-slate-100 pb-4 sm:flex-row sm:items-end sm:justify-between">
      <div>
        <p className="text-xs font-bold uppercase tracking-[0.16em] text-[#0E7C6B]">Hàng chờ quyết định</p>
      ...
      <span className="inline-flex w-fit items-center gap-2 rounded-full bg-emerald-50 px-3 py-1.5 text-sm font-semibold text-[#0A5C50]"><span className="h-2 w-2 rounded-full bg-[#0E7C6B]" />{isLoading ? 'Đang đồng bộ' : cases.length + ' yêu cầu chờ'}</span>
    </header>

    <p className="mt-4 rounded-2xl border border-emerald-100 bg-emerald-50 p-3 text-sm leading-6 text-emerald-950 sm:p-4"><span className="font-bold">Quy tắc cố định:</span> Chủ tịch bắt buộc + ít nhất 2/4 Ủy viên cùng một quyết định. Bốn Ủy viên không có Chủ tịch vẫn chưa đạt.</p>
    ...
    {isLoading ? <div aria-label="Đang tải hàng chờ giải ngân" className="mt-5 space-y-4">
      {Array.from({ length: 2 }).map((_, index) => <div key={index} className="animate-pulse rounded-2xl border border-slate-100 p-4"><div className="flex justify-between gap-4"><div className="h-5 w-36 rounded bg-slate-200" /><div className="h-8 w-28 rounded-full bg-emerald-100" /></div><div className="mt-3 h-4 w-2/3 rounded bg-slate-200" /><div className="mt-5 h-44 rounded-xl bg-slate-100" /><div className="mt-4 h-24 rounded-xl bg-slate-100" /></div>)}
    </div> : null}

    {!isLoading && cases.length === 0 ? <div className="mt-5 rounded-2xl border border-dashed border-emerald-200 bg-emerald-50/60 px-5 py-10 text-center">
      ...
      return <article key={item.disbursement.requestId} className="min-w-0 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm sm:p-5">
        <div className="flex flex-col gap-4 border-b border-slate-100 pb-4 sm:flex-row sm:items-start sm:justify-between">
          <div className="min-w-0">
            <div className="flex flex-wrap items-center gap-2"><span className="rounded-full bg-emerald-100 px-2.5 py-1 text-xs font-bold text-[#0A5C50]">Yêu cầu chờ duyệt</span><span className="rounded-full bg-slate-100 px-2.5 py-1 text-xs font-semibold text-slate-600">{item.disbursement.requestMode}</span></div>
          ...
          <div className="w-fit shrink-0 rounded-2xl bg-gradient-to-br from-[#0E7C6B] to-[#0A5C50] px-4 py-3 text-white shadow-[0_12px_24px_-16px_rgba(14,124,107,0.72)]">
            <p className="text-[11px] font-semibold uppercase tracking-wide text-emerald-100">Số tiền đề nghị</p>
            <p className="mt-1 text-lg font-bold">{item.disbursement.amount.toLocaleString('vi-VN')} <span className="text-sm text-emerald-100">token</span></p>
          </div>
        </div>
        ...
        <label className="mt-4 block text-sm font-bold text-slate-800">Lý do biểu quyết
          <textarea aria-label={'Lý do cho ' + item.disbursement.requestId} value={reasonByRequestId[item.disbursement.requestId] || ''} onChange={event => setReasonByRequestId(current => ({ ...current, [item.disbursement.requestId]: event.target.value }))} className="mt-2 min-h-28 w-full rounded-xl border border-slate-200 bg-white p-3 text-base font-normal leading-6 outline-none transition-colors placeholder:text-slate-400 focus:border-[#0E7C6B] focus:ring-2 focus:ring-emerald-100" placeholder="Nêu căn cứ biểu quyết (ít nhất 10 ký tự)" maxLength={500} />
        </label>
    ...
    {!isLoading && nextCursor ? <div className="mt-5 flex justify-center"><button type="button" onClick={() => void loadCases(nextCursor, true)} className="min-h-11 rounded-xl border border-emerald-200 bg-emerald-50 px-4 py-2 text-sm font-bold text-[#0A5C50] transition-colors hover:bg-emerald-100">Tải thêm yêu cầu</button></div> : null}
  </section>;
}
